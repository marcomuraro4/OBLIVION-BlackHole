...
To enable the communication between the Vue app and Unreal Engine running on separate machines, go to **index.js** in **node-server** directory and set the correct remote IP address and port (lines 27 and 28 respectively). Then, set the same address in Unreal Engine project within BP_BlackHole Blueprint.
